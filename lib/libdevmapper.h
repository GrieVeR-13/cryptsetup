...
void dm_log_with_errno_init(dm_log_with_errno_fn fn){ abort(); }
 ...
 */
int dm_log_is_non_default(void){ abort(); }

/*
 * Number of devices currently in suspended state (via the library).
 */
int dm_get_suspended_counter(void){ abort(); }
...
struct dm_task *dm_task_create(int type){ abort(); }
void dm_task_destroy(struct dm_task *dmt){ abort(); }

int dm_task_set_name(struct dm_task *dmt, const char *name){ abort(); }
int dm_task_set_uuid(struct dm_task *dmt, const char *uuid){ abort(); }
...
int dm_get_library_version(char *version, size_t size){ abort(); }
int dm_task_get_driver_version(struct dm_task *dmt, char *version, size_t size){ abort(); }
int dm_task_get_info(struct dm_task *dmt, struct dm_info *dmi){ abort(); }
 ...
 */
const char *dm_task_get_uuid(const struct dm_task *dmt){ abort(); }

struct dm_deps *dm_task_get_deps(struct dm_task *dmt){ abort(); }
struct dm_versions *dm_task_get_versions(struct dm_task *dmt){ abort(); }
 ...
 */
const char *dm_task_get_name(const struct dm_task *dmt){ abort(); }
struct dm_names *dm_task_get_names(struct dm_task *dmt){ abort(); }

int dm_task_set_ro(struct dm_task *dmt){ abort(); }
int dm_task_set_newname(struct dm_task *dmt, const char *newname){ abort(); }
int dm_task_set_newuuid(struct dm_task *dmt, const char *newuuid){ abort(); }
int dm_task_set_minor(struct dm_task *dmt, int minor){ abort(); }
int dm_task_set_major(struct dm_task *dmt, int major){ abort(); }
int dm_task_set_major_minor(struct dm_task *dmt, int major, int minor, int allow_default_major_fallback){ abort(); }
int dm_task_set_uid(struct dm_task *dmt, uid_t uid){ abort(); }
int dm_task_set_gid(struct dm_task *dmt, gid_t gid){ abort(); }
int dm_task_set_mode(struct dm_task *dmt, mode_t mode){ abort(); }
int dm_task_set_cookie(struct dm_task *dmt, uint32_t *cookie, uint16_t flags){ abort(); }
int dm_task_set_event_nr(struct dm_task *dmt, uint32_t event_nr){ abort(); }
int dm_task_set_geometry(struct dm_task *dmt, const char *cylinders, const char *heads, const char *sectors, const char *start){ abort(); }
int dm_task_set_message(struct dm_task *dmt, const char *message){ abort(); }
int dm_task_set_sector(struct dm_task *dmt, uint64_t sector){ abort(); }
int dm_task_no_flush(struct dm_task *dmt){ abort(); }
int dm_task_no_open_count(struct dm_task *dmt){ abort(); }
int dm_task_skip_lockfs(struct dm_task *dmt){ abort(); }
int dm_task_query_inactive_table(struct dm_task *dmt){ abort(); }
int dm_task_suppress_identical_reload(struct dm_task *dmt){ abort(); }
int dm_task_secure_data(struct dm_task *dmt){ abort(); }
int dm_task_retry_remove(struct dm_task *dmt){ abort(); }

/*
 * Enable checks for common mistakes such as issuing ioctls in an unsafe order.
 */
int dm_task_enable_checks(struct dm_task *dmt){ abort(); }

typedef enum {
	DM_ADD_NODE_ON_RESUME, /* add /dev/mapper node with dmsetup resume */
	DM_ADD_NODE_ON_CREATE  /* add /dev/mapper node with dmsetup create */
} dm_add_node_t;
int dm_task_set_add_node(struct dm_task *dmt, dm_add_node_t add_node){ abort(); }
...
int dm_task_set_read_ahead(struct dm_task *dmt, uint32_t read_ahead,
			   uint32_t read_ahead_flags){ abort(); }
uint32_t dm_task_get_read_ahead(const struct dm_task *dmt,
				uint32_t *read_ahead){ abort(); }
...
int dm_task_add_target(struct dm_task *dmt,
		       uint64_t start,
		       uint64_t size, const char *ttype, const char *params){ abort(); }

/*
 * Format major/minor numbers correctly for input to driver.
 */
#define DM_FORMAT_DEV_BUFSIZE	13	/* Minimum bufsize to handle worst case. */
int dm_format_dev(char *buf, int bufsize, uint32_t dev_major, uint32_t dev_minor){ abort(); }

/* Use this to retrive target information returned from a STATUS call */
void *dm_get_next_target(struct dm_task *dmt,
			 void *next, uint64_t *start, uint64_t *length,
			 char **target_type, char **params){ abort(); }
...
int dm_get_status_thin_pool(struct dm_pool *mem, const char *params,
			    struct dm_status_thin_pool **status){ abort(); }
...
int dm_get_status_thin(struct dm_pool *mem, const char *params,
		       struct dm_status_thin **status){ abort(); }

/*
 * Call this to actually run the ioctl.
 */
int dm_task_run(struct dm_task *dmt){ abort(); }

/*
 * Call this to make or remove the device nodes associated with previously
 * issued commands.
 */
void dm_task_update_nodes(void){ abort(); }
 ...
 */
int dm_set_name_mangling_mode(dm_string_mangling_t name_mangling){ abort(); }
dm_string_mangling_t dm_get_name_mangling_mode(void){ abort(); }
 ...
 */
char *dm_task_get_name_mangled(const struct dm_task *dmt){ abort(); }
char *dm_task_get_name_unmangled(const struct dm_task *dmt){ abort(); }
char *dm_task_get_uuid_mangled(const struct dm_task *dmt){ abort(); }
char *dm_task_get_uuid_unmangled(const struct dm_task *dmt){ abort(); }

/*
 * Configure the device-mapper directory
 */
int dm_set_dev_dir(const char *dir){ abort(); }
const char *dm_dir(void){ abort(); }

/*
 * Configure sysfs directory, /sys by default
 */
int dm_set_sysfs_dir(const char *dir){ abort(); }
const char *dm_sysfs_dir(void){ abort(); }
 ...
 */
int dm_set_uuid_prefix(const char *uuid_prefix){ abort(); }
const char *dm_uuid_prefix(void){ abort(); }

/*
 * Determine whether a major number belongs to device-mapper or not.
 */
int dm_is_dm_major(uint32_t major){ abort(); }
...
int dm_device_get_name(uint32_t major, uint32_t minor,
		       ...
		       char *buf, size_t buf_size){ abort(); }
 ...
 */
int dm_device_has_holders(uint32_t major, uint32_t minor){ abort(); }

/*
 * Determine whether a device contains mounted filesystem.
 * If sysfs is not used (or configured incorrectly), returns 0.
 */
int dm_device_has_mounted_fs(uint32_t major, uint32_t minor){ abort(); }
 ...
 */
int dm_mknodes(const char *name){ abort(); }
int dm_driver_version(char *version, size_t size){ abort(); }
 ...
 * parents/children.
 */
struct dm_tree *dm_tree_create(void){ abort(); }
void dm_tree_free(struct dm_tree *tree){ abort(); }

/*
 * Add nodes to the tree for a given device and all the devices it uses.
 */
int dm_tree_add_dev(struct dm_tree *tree, uint32_t major, uint32_t minor){ abort(); }
int dm_tree_add_dev_with_udev_flags(struct dm_tree *tree, uint32_t major,
				    uint32_t minor, uint16_t udev_flags){ abort(); }
...
struct dm_tree_node *dm_tree_next_child(void **handle,
					...
					uint32_t inverted){ abort(); }

/*
 * Get properties of a node.
 */
const char *dm_tree_node_get_name(const struct dm_tree_node *node){ abort(); }
const char *dm_tree_node_get_uuid(const struct dm_tree_node *node){ abort(); }
const struct dm_info *dm_tree_node_get_info(const struct dm_tree_node *node){ abort(); }
void *dm_tree_node_get_context(const struct dm_tree_node *node){ abort(); }
int dm_tree_node_size_changed(const struct dm_tree_node *dnode){ abort(); }

/*
 * Returns the number of children of the given node (excluding the root node).
 * Set inverted for the number of parents.
 */
int dm_tree_node_num_children(const struct dm_tree_node *node, uint32_t inverted){ abort(); }
 ...
 */ 
void dm_tree_skip_lockfs(struct dm_tree_node *dnode){ abort(); }
 ...
 */
void dm_tree_use_no_flush_suspend(struct dm_tree_node *dnode){ abort(); }

/*
 * Retry removal of each device if not successful.
 */
void dm_tree_retry_remove(struct dm_tree_node *dnode){ abort(); }
...
int dm_tree_node_add_snapshot_origin_target(struct dm_tree_node *dnode,
					       uint64_t size,
					       const char *origin_uuid){ abort(); }
int dm_tree_node_add_snapshot_target(struct dm_tree_node *node,
					uint64_t size,
					const char *origin_uuid,
					const char *cow_uuid,
					int persistent,
					uint32_t chunk_size){ abort(); }
...
int dm_tree_node_add_replicator_target(struct dm_tree_node *node,
				       ...
				       uint32_t fall_behind_ios){ abort(); }
...
int dm_tree_node_add_replicator_dev_target(struct dm_tree_node *node,
					   ...
					   uint32_t slog_region_size){ abort(); }
...
int dm_tree_node_add_thin_pool_target(struct dm_tree_node *node,
				      ...
				      unsigned skip_block_zeroing){ abort(); }
...
int dm_tree_node_add_thin_pool_message(struct dm_tree_node *node,
				       ...
				       uint64_t id1, uint64_t id2){ abort(); }
...
int dm_tree_node_add_thin_target(struct dm_tree_node *node,
				 uint64_t size,
				 const char *pool_uuid,
				 uint32_t device_id){ abort(); }

int dm_tree_node_set_thin_external_origin(struct dm_tree_node *node,
					  const char *external_uuid){ abort(); }

void dm_tree_node_set_udev_flags(struct dm_tree_node *node, uint16_t udev_flags){ abort(); }

void dm_tree_node_set_presuspend_node(struct dm_tree_node *node,
				      struct dm_tree_node *presuspend_node){ abort(); }

int dm_tree_node_add_target_area(struct dm_tree_node *node,
				    const char *dev_name,
				    const char *dlid,
				    uint64_t offset){ abort(); }

/*
 * Only for temporarily-missing raid devices where changes are tracked.
 */
int dm_tree_node_add_null_area(struct dm_tree_node *node, uint64_t offset){ abort(); }
...
void dm_tree_node_set_callback(struct dm_tree_node *node,
			       dm_node_callback_fn cb, void *cb_data){ abort(); }

void dm_tree_set_cookie(struct dm_tree_node *node, uint32_t cookie){ abort(); }
uint32_t dm_tree_get_cookie(struct dm_tree_node *node){ abort(); }
...
void *dm_malloc_aux(size_t s, const char *file, int line)
	__attribute__((malloc)) __attribute__((__warn_unused_result__)){ abort(); }
void *dm_malloc_aux_debug(size_t s, const char *file, int line)
	__attribute__((__warn_unused_result__)){ abort(); }
...
char *dm_strdup_aux(const char *str, const char *file, int line)
	__attribute__((malloc)) __attribute__((__warn_unused_result__)){ abort(); }
void dm_free_aux(void *p){ abort(); }
void *dm_realloc_aux(void *p, unsigned int s, const char *file, int line)
	__attribute__((__warn_unused_result__)){ abort(); }
int dm_dump_memory_debug(void){ abort(); }
void dm_bounds_check_debug(void){ abort(); }
...
void dm_pool_destroy(struct dm_pool *p){ abort(); }
...
void *dm_pool_alloc_aligned(struct dm_pool *p, size_t s, unsigned alignment)
	__attribute__((__warn_unused_result__)){ abort(); }
void dm_pool_empty(struct dm_pool *p){ abort(); }
void dm_pool_free(struct dm_pool *p, void *ptr){ abort(); }
...
int dm_pool_locked(struct dm_pool *p){ abort(); }
/* mark pool as locked */
 ...
 */
int dm_pool_begin_object(struct dm_pool *p, size_t hint){ abort(); }
int dm_pool_grow_object(struct dm_pool *p, const void *extra, size_t delta){ abort(); }
void *dm_pool_end_object(struct dm_pool *p){ abort(); }
void dm_pool_abandon_object(struct dm_pool *p){ abort(); }
...
dm_bitset_t dm_bitset_create(struct dm_pool *mem, unsigned num_bits){ abort(); }
void dm_bitset_destroy(dm_bitset_t bs){ abort(); }

int dm_bitset_equal(dm_bitset_t in1, dm_bitset_t in2){ abort(); }

void dm_bit_and(dm_bitset_t out, dm_bitset_t in1, dm_bitset_t in2){ abort(); }
void dm_bit_union(dm_bitset_t out, dm_bitset_t in1, dm_bitset_t in2){ abort(); }
int dm_bit_get_first(dm_bitset_t bs){ abort(); }
int dm_bit_get_next(dm_bitset_t bs, int last_bit){ abort(); }
...
struct dm_hash_table *dm_hash_create(unsigned size_hint)
	__attribute__((__warn_unused_result__)){ abort(); }
void dm_hash_destroy(struct dm_hash_table *t){ abort(); }
void dm_hash_wipe(struct dm_hash_table *t){ abort(); }

void *dm_hash_lookup(struct dm_hash_table *t, const char *key){ abort(); }
int dm_hash_insert(struct dm_hash_table *t, const char *key, void *data){ abort(); }
void dm_hash_remove(struct dm_hash_table *t, const char *key){ abort(); }

void *dm_hash_lookup_binary(struct dm_hash_table *t, const void *key, uint32_t len){ abort(); }
int dm_hash_insert_binary(struct dm_hash_table *t, const void *key, uint32_t len,
		       void *data){ abort(); }
void dm_hash_remove_binary(struct dm_hash_table *t, const void *key, uint32_t len){ abort(); }

unsigned dm_hash_get_num_entries(struct dm_hash_table *t){ abort(); }
void dm_hash_iter(struct dm_hash_table *t, dm_hash_iterate_fn f){ abort(); }

char *dm_hash_get_key(struct dm_hash_table *t, struct dm_hash_node *n){ abort(); }
void *dm_hash_get_data(struct dm_hash_table *t, struct dm_hash_node *n){ abort(); }
struct dm_hash_node *dm_hash_get_first(struct dm_hash_table *t){ abort(); }
struct dm_hash_node *dm_hash_get_next(struct dm_hash_table *t, struct dm_hash_node *n){ abort(); }
...
void dm_list_init(struct dm_list *head){ abort(); }

/*
 * Insert an element before 'head'.
 * If 'head' is the list head, this adds an element to the end of the list.
 */
void dm_list_add(struct dm_list *head, struct dm_list *elem){ abort(); }

/*
 * Insert an element after 'head'.
 * If 'head' is the list head, this adds an element to the front of the list.
 */
void dm_list_add_h(struct dm_list *head, struct dm_list *elem){ abort(); }
 ...
 */
void dm_list_del(struct dm_list *elem){ abort(); }

/*
 * Remove an element from existing list and insert before 'head'.
 */
void dm_list_move(struct dm_list *head, struct dm_list *elem){ abort(); }

/*
 * Join 'head1' to the end of 'head'.
 */
void dm_list_splice(struct dm_list *head, struct dm_list *head1){ abort(); }

/*
 * Is the list empty?
 */
int dm_list_empty(const struct dm_list *head){ abort(); }

/*
 * Is this the first element of the list?
 */
int dm_list_start(const struct dm_list *head, const struct dm_list *elem){ abort(); }

/*
 * Is this the last element of the list?
 */
int dm_list_end(const struct dm_list *head, const struct dm_list *elem){ abort(); }

/*
 * Return first element of the list or NULL if empty
 */
struct dm_list *dm_list_first(const struct dm_list *head){ abort(); }

/*
 * Return last element of the list or NULL if empty
 */
struct dm_list *dm_list_last(const struct dm_list *head){ abort(); }

/*
 * Return the previous element of the list, or NULL if we've reached the start.
 */
struct dm_list *dm_list_prev(const struct dm_list *head, const struct dm_list *elem){ abort(); }

/*
 * Return the next element of the list, or NULL if we've reached the end.
 */
struct dm_list *dm_list_next(const struct dm_list *head, const struct dm_list *elem){ abort(); }
 ...
 */
unsigned int dm_list_size(const struct dm_list *head){ abort(); }
 ...
 */
int dm_prepare_selinux_context(const char *path, mode_t mode){ abort(); }
/*
 * Set SELinux context for existing file system object.
 */
int dm_set_selinux_context(const char *path, mode_t mode){ abort(); }
...
int dm_split_lvm_name(struct dm_pool *mem, const char *dmname,
		      char **vgname, char **lvname, char **layer){ abort(); }
...
int dm_snprintf(char *buf, size_t bufsize, const char *format, ...)
    __attribute__ ((format(printf, 3, 4))){ abort(); }

/*
 * Returns pointer to the last component of the path.
 */
const char *dm_basename(const char *path){ abort(); }

/*
 * Returns number of occurrences of 'c' in 'str' of length 'size'.
 */
unsigned dm_count_chars(const char *str, size_t len, const int c){ abort(); }

/*
 * Length of string after escaping double quotes and backslashes.
 */
size_t dm_escaped_len(const char *str){ abort(); }

/*
 * <vg>-<lv>-<layer> or if !layer just <vg>-<lv>.
 */
char *dm_build_dm_name(struct dm_pool *mem, const char *vgname,
		       const char *lvname, const char *layer){ abort(); }
char *dm_build_dm_uuid(struct dm_pool *mem, const char *prefix, const char *lvid, const char *layer){ abort(); }

/*
 * Copies a string, quoting double quotes with backslashes.
 */
char *dm_escape_double_quotes(char *out, const char *src){ abort(); }

/*
 * Undo quoting in situ.
 */
void dm_unescape_double_quotes(char *src){ abort(); }
...
void dm_unescape_colons_and_at_signs(char *src,
				     ...
				     char **substr_first_unquoted_at_sign){ abort(); }
 ...
 */
int dm_strncpy(char *dest, const char *src, size_t n){ abort(); }
 ...
 */
int dm_create_dir(const char *dir){ abort(); }

int dm_is_empty_dir(const char *dir){ abort(); }
 ...
 */
int dm_fclose(FILE *stream){ abort(); }
...
int dm_asprintf(char **buf, const char *format, ...)
    __attribute__ ((format(printf, 2, 3))){ abort(); }
int dm_vasprintf(char **buf, const char *format, va_list ap){ abort(); }
 ...
 */
int dm_create_lockfile(const char* lockfile){ abort(); }
 ...
 */
int dm_daemon_is_running(const char* lockfile){ abort(); }
 ...
 * or -1 if none match.
 */
int dm_regex_match(struct dm_regex *regex, const char *s){ abort(); }
 ...
 */
uint32_t dm_regex_fingerprint(struct dm_regex *regex){ abort(); }
...
struct dm_report *dm_report_init(uint32_t *report_types,
				 ...
				 void *private_data){ abort(); }
int dm_report_object(struct dm_report *rh, void *object){ abort(); }
int dm_report_output(struct dm_report *rh){ abort(); }
void dm_report_free(struct dm_report *rh){ abort(); }
...
int dm_report_set_output_field_name_prefix(struct dm_report *rh,
					   const char *report_prefix){ abort(); }
...
int dm_config_parse(struct dm_config_tree *cft, const char *start, const char *end){ abort(); }

void *dm_config_get_custom(struct dm_config_tree *cft){ abort(); }
void dm_config_set_custom(struct dm_config_tree *cft, void *custom){ abort(); }
 ...
 */
struct dm_config_tree *dm_config_remove_cascaded_tree(struct dm_config_tree *cft){ abort(); }

void dm_config_destroy(struct dm_config_tree *cft){ abort(); }

typedef int (*dm_putline_fn)(const char *line, void *baton);
/* Write the node and any subsequent siblings it has. */
int dm_config_write_node(const struct dm_config_node *cn, dm_putline_fn putline, void *baton){ abort(); }
/* Write given node only without subsequent siblings. */
int dm_config_write_one_node(const struct dm_config_node *cn, dm_putline_fn putline, void *baton){ abort(); }

struct dm_config_node *dm_config_find_node(const struct dm_config_node *cn, const char *path){ abort(); }
int dm_config_has_node(const struct dm_config_node *cn, const char *path){ abort(); }
const char *dm_config_find_str(const struct dm_config_node *cn, const char *path, const char *fail){ abort(); }
const char *dm_config_find_str_allow_empty(const struct dm_config_node *cn, const char *path, const char *fail){ abort(); }
int dm_config_find_int(const struct dm_config_node *cn, const char *path, int fail){ abort(); }
int64_t dm_config_find_int64(const struct dm_config_node *cn, const char *path, int64_t fail){ abort(); }
float dm_config_find_float(const struct dm_config_node *cn, const char *path, float fail){ abort(); }

const struct dm_config_node *dm_config_tree_find_node(const struct dm_config_tree *cft, const char *path){ abort(); }
const char *dm_config_tree_find_str(const struct dm_config_tree *cft, const char *path, const char *fail){ abort(); }
const char *dm_config_tree_find_str_allow_empty(const struct dm_config_tree *cft, const char *path, const char *fail){ abort(); }
int dm_config_tree_find_int(const struct dm_config_tree *cft, const char *path, int fail){ abort(); }
int64_t dm_config_tree_find_int64(const struct dm_config_tree *cft, const char *path, int64_t fail){ abort(); }
float dm_config_tree_find_float(const struct dm_config_tree *cft, const char *path, float fail){ abort(); }
int dm_config_tree_find_bool(const struct dm_config_tree *cft, const char *path, int fail){ abort(); }

/*
 * Understands (0, ~0), (y, n), (yes, no), (on,
 * off), (true, false).
 */
int dm_config_find_bool(const struct dm_config_node *cn, const char *path, int fail){ abort(); }

int dm_config_get_uint32(const struct dm_config_node *cn, const char *path, uint32_t *result){ abort(); }
int dm_config_get_uint64(const struct dm_config_node *cn, const char *path, uint64_t *result){ abort(); }
int dm_config_get_str(const struct dm_config_node *cn, const char *path, const char **result){ abort(); }
int dm_config_get_list(const struct dm_config_node *cn, const char *path, const struct dm_config_value **result){ abort(); }
int dm_config_get_section(const struct dm_config_node *cn, const char *path, const struct dm_config_node **result){ abort(); }

unsigned dm_config_maybe_section(const char *str, unsigned len){ abort(); }

const char *dm_config_parent_name(const struct dm_config_node *n){ abort(); }

struct dm_config_node *dm_config_clone_node_with_mem(struct dm_pool *mem, const struct dm_config_node *node, int siblings){ abort(); }
struct dm_config_node *dm_config_create_node(struct dm_config_tree *cft, const char *key){ abort(); }
struct dm_config_value *dm_config_create_value(struct dm_config_tree *cft){ abort(); }
struct dm_config_node *dm_config_clone_node(struct dm_config_tree *cft, const struct dm_config_node *cn, int siblings){ abort(); }

struct dm_pool *dm_config_memory(struct dm_config_tree *cft){ abort(); }
...
int dm_cookie_supported(void){ abort(); }

/*
 * Udev synchronisation functions.
 */
void dm_udev_set_sync_support(int sync_with_udev){ abort(); }
int dm_udev_get_sync_support(void){ abort(); }
void dm_udev_set_checking(int checking){ abort(); }
int dm_udev_get_checking(void){ abort(); }

/*
 * Default value to get new auto generated cookie created
 */
#define DM_COOKIE_AUTO_CREATE 0
int dm_udev_create_cookie(uint32_t *cookie){ abort(); }
int dm_udev_complete(uint32_t cookie);
int dm_udev_wait(uint32_t cookie){ abort(); }
